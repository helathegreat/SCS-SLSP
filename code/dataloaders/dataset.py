...
class BrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        npmask[np.where(npmask==4)] = 3
        nplabel = npmask

        sample = {'image': npimage, 'label':  nplabel}
...
class LAHeart(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._base_dir + "/2018_Atrial_Segmentation_Challenge_COMPLETE_DATASET/Training_Set/" + image_name + "/mri_norm2.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label}
        if self.transform:
...
class TwoStreamBatchSampler(Sampler):
    ...
    def __init__(self, primary_indices, secondary_indices, batch_size, secondary_batch_size):
        self.primary_indices = primary_indices#68片是训练数据
        self.secondary_indices = secondary_indices#1244 是所有的数据
...
def iterate_once(iterable):
    return np.random.permutation(iterable)#把68组slice打乱
